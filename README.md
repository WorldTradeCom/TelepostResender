...
"ai_filters": []
```
При подключении [NeuroHub](https://github.com/dub1401/NeuroHub) при помощи AI будет произведена попытка обнаружить перечисленные элементы. При обнаружении пост пропускается.
___
```JSON
"remove_html_tags": []
```
...
"buzzerator_requests": []
```
Здесь можно определить дополнительные строки, добавляемые к запросу на перевод через [BuzzerMutarji](https://github.com/WorldTradeCom/BuzzerMutarji).
___
```JSON
